...
- `New_FRONTEND_GUIDELINES.md`（仅在执行新版 UI/UX 重绘、视觉重构或相关方案评审时必须参考）
...
4. `TECH_STACK.md`
5. `FRONTEND_GUIDELINES.md` / `New_FRONTEND_GUIDELINES.md`（按当前任务是否为旧版维护或新版重绘选择 active UI 文档）
...
实现前端时，必须直接引用当前 active UI 文档中的设计令牌，不得自行发明新主色。

- 旧版维护、旧版回归和未确认新版方案时，active UI 文档为 `FRONTEND_GUIDELINES.md`。
- 新版 UI/UX 重绘方案经用户确认后，active UI 文档为 `New_FRONTEND_GUIDELINES.md` 中的“当前 UI/UX 重绘实施方案（待确认）”及后续正式化内容。

### 8.1 核心颜色

以下颜色属于旧版 UI/UX 设计归档，适用于旧版维护与回归比对：
...
若执行新版 UI/UX 重绘，应使用 `New_FRONTEND_GUIDELINES.md` 中定义的 `calm-*`、`ink-*`、`surface-*` 与语义色 token。
