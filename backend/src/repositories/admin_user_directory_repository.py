...
class AdminUserDirectoryRepository:
    ...
    def count_students_by_risk_status(
        self,
        *,
        show_seeded_cases: bool,
    ) -> dict[StudentRiskStatus, int]:
        """Return student counts grouped by aggregate risk status."""
        statement = select(StudentUser.risk_status, func.count()).group_by(
            StudentUser.risk_status
        )
        if not show_seeded_cases:
            statement = statement.where(exclude_seeded_students_clause(StudentUser.id))
    ...
    def count_active_focus_entries_by_student(
        self,
        *,
        show_seeded_cases: bool,
    ) -> dict[int, int]:
    ...
    def count_open_alert_cases_by_student(
        self,
        *,
        show_seeded_cases: bool,
    ) -> dict[int, int]:
